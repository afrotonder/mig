...
4. In the project folder create a directory called newImagesJSON and another one called final.
5. Extract downloaded ZIP and drop the files into  newImagesJSON.
6. Take the IPFS image URI, extract the final 3 characters and the / before the file name like so (extract this 6im/ from this ..6im/1.json )
7. Replace the URI extract in line 53 of the code ( .split('6im/')[1] ) with your URI extract (replace 6im/ with your extract)
8. Run script with python3 mig.py
